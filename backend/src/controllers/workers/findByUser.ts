...
const findByUser = async (req, res) => {
  ...
  const results = await WorkerRepository.query(`select * from registration.worker w where w.\"userId\" = ${req.params.id};`)
  
  if (results.length <= 0) {
    return res.status(404).json({
      messagem: `O usuário id ${req.params.id} não é membro de nenhum departamento.`
    })
  }
